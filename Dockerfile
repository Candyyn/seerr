...
RUN --mount=type=cache,id=pnpm,target=/pnpm/store CI=true pnpm install --prod
...
RUN --mount=type=cache,id=pnpm,target=/pnpm/store CYPRESS_INSTALL_BINARY=0 pnpm install
